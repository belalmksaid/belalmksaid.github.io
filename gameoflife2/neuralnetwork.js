...
function NeuralNetwork(ni, no, nhl, npr) {
	this.numInputs = ni;
	this.numOutputs = no;
	this.numHiddenLayers = nhl;
	this.NPR = npr;
...
function GenePool() {
	...
	this.bestFitness = function() {
		var best = -10000000000000000;
		var bestIndex = -1;
		for(var i = 0; i < this.genes.length; i++) {
			if(this.genes[i].fitness > best) {
				best = this.genes[i].fitness;
				bestIndex = i;
			}
		}
		return new Array(best, bestIndex);
	}
